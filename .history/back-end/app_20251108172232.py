...
def get_market_total(marketID, currency_type):
    ...
    key = f"market:{marketID}:total{currency_type}"
...
def get_user_wallet(userID):
    ...
    return {"userID": wallet_data["userID"], "walletUsd": wallet_data["walletUsd"], "wallet_coins": wallet_data["wallet_coins"]}
...
def buy_coins(userID, num_coins, current_price, marketID = 1):
    ...
    r.hset(key, mapping=wallet_data)
    return {"cost": cost, "new_usd": wallet_data["walletUsd"], "new_coins": wallet_data["wallet_coins"], "current_price": current_price, "num_coins": num_coins}
...
def sell_coins(userID, num_coins, current_price, marketID = 1):
    ...
    r.hset(key, mapping=wallet_data)
    return {"revenue": revenue, "new_usd": wallet_data["walletUsd"], "new_coins": wallet_data["wallet_coins"], "current_price": current_price, "num_coins": num_coins}
...
def on_key(event):
    if event.key == 'b':
        print(buy_coins(123, 1, float(get_price_history(1)[-1])))
    elif event.key == 's':
        print(sell_coins(123, 1, float(get_price_history(1)[-1])))
