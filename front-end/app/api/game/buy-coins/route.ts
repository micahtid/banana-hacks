...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Buy Coins] Calling backend API:', `${BACKEND_URL}/api/game/buy-coins`);
    
    let response;
    try {
      response = await fetch(`${BACKEND_URL}/api/game/buy-coins`, {
        method: 'POST',
        headers: {
      ...
      console.error('[Buy Coins] Failed to connect to backend:', fetchError);
      return NextResponse.json(
    ...
    console.log('[Buy Coins] Backend response status:', response.status);
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.error('[Buy Coins] Raw response from backend:', rawText);
        errorData = JSON.parse(rawText);
      } catch (e) {
        console.error('[Buy Coins] Could not parse backend response, raw text:', rawText);
        errorData = { detail: rawText || 'Unknown error from backend' };
      }
      console.error('[Buy Coins] Parsed error data:', errorData);
      ...
      return NextResponse.json(
        { error: errorData.error || errorData.message || 'Failed to buy coins' },
        { status: response.status }
      );
    }

    const data = await response.json();
    console.log('[Buy Coins] Success:', data);
    ...
    console.error('[Buy Coins] Unexpected error:', error);
    return NextResponse.json(
      { 
